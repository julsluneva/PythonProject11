...
def test_filter_by_currency_missing_code():
    ...
    transactions = [{"id": 1, "state": "EXECUTED", "operationAmount": {"amount": "100", "currency": {"name": "USD"}}}]
    result = list(filter_by_currency(transactions, "USD"))
    assert len(result) == 0
...
def test_filter_by_currency_invalid_currency_structure():
    ...
    transactions = [{"operationAmount": {"currency": "USD"}}]  # Должно быть dict, а не str
    result = list(filter_by_currency(transactions, "USD"))
    assert len(result) == 0
...
def test_filter_by_currency_missing_code_key():
    ...
    transactions = [{"operationAmount": {"currency": {"name": "USD"}}}]  # Нет code
    result = list(filter_by_currency(transactions, "USD"))
    assert len(result) == 0
