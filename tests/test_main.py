...
from main import get_file_path, get_user_choice, read_json_file, get_status_from_user
...
class TestMainFunction:
    ...
    @patch('main.input')
    def test_main_full_flow_json(self, mock_input, mock_get_user_choice,
                                 mock_get_status, mock_read_json,
                                 mock_load_csv, mock_load_excel,
                                 mock_sort_date, mock_process_operations,
                                 mock_process_search, mock_filter_state,
                                 mock_filter_currency, mock_display,
                                 mock_transactions):
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_main_csv_file(self, mock_input, mock_get_user_choice,
                           mock_get_status, mock_read_json,
                           mock_load_csv, mock_load_excel,
                           mock_sort_date, mock_process_operations,
                           mock_process_search, mock_filter_state,
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_main_excel_file(self, mock_input, mock_get_user_choice,
                             mock_get_status, mock_read_json,
                             mock_load_csv, mock_load_excel,
                             mock_sort_date, mock_process_operations,
                             mock_process_search, mock_filter_state,
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_main_with_sorting(self, mock_input, mock_get_user_choice,
                               mock_get_status, mock_read_json,
                               mock_filter_state, mock_display,
                               mock_transactions):
        ...
        with patch('main.sort_by_date') as mock_sort:
            mock_sort.return_value = sorted(executed_transactions,
                                            key=lambda x: x["date"],
                                            reverse=True)
            main()
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_main_with_rub_only(self, mock_input, mock_get_user_choice,
                                mock_get_status, mock_read_json,
                                mock_filter_state, mock_display,
                                mock_transactions):
        ...
        with patch('main.filter_by_currency') as mock_filter_currency:
            rub_transactions = [t for t in executed_transactions
                                if t["operationAmount"]["currency"]["code"] == "RUB"]
            mock_filter_currency.return_value = rub_transactions
            main()

            mock_filter_currency.assert_called_once_with(
                executed_transactions, rub_only=True
            )
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_main_with_search(self, mock_input, mock_get_user_choice,
                              mock_get_status, mock_read_json,
                              mock_filter_state, mock_display,
                              mock_transactions):
        ...
        with patch('main.process_bank_search') as mock_search:
            search_results = [t for t in executed_transactions
                              if 'перевод' in t["description"].lower()]
            mock_search.return_value = search_results
            main()

            mock_search.assert_called_once_with(
                executed_transactions, 'перевод'
            )
            ...
            with patch('main.get_status_from_user') as mock_get_status:
                ...
                with patch('main.filter_by_state') as mock_filter_state:
                    ...
                    with patch('main.print') as mock_print:
                        ...
                        found = any("Не найдено ни одной транзакции" in str(call)
...
class TestIntegrationScenarios:
    ...
    @patch('main.get_user_choice')
    @patch('main.input')
    def test_complete_workflow(self, mock_input, mock_get_user_choice,
                               mock_get_status, mock_load_csv,
                               mock_filter_state, mock_sort_date,
                               mock_filter_currency, mock_process_operations,
                               mock_process_search, mock_display,
                               mock_csv_transactions):
        ...
        mock_filter_state.return_value = executed_transactions
        mock_sort_date.return_value = sorted(executed_transactions,
                                             key=lambda x: x["date"],
                                             reverse=True)
        rub_transactions = [t for t in executed_transactions
                            if t.get("currency_code") == "RUB" or
                            t.get("currency_name") == "руб."]
        mock_filter_currency.return_value = rub_transactions
        search_results = [t for t in rub_transactions
                          if 'перевод' in t["description"].lower()]
